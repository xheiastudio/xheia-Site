...
| `/track-record` | Header, intro line, anonymization disclaimer + grid of CaseStudyCard (real entries as of the real-content pass), Footer |
...
`Xheia (mark+wordmark)` — `Track Record` — `Approach` — `About` — `Contact` — theme toggle — one CTA button. No dropdowns, no secondary items.
...
- **Hero** — mark, tagline headline, one-line outcome-framed subhead, CTA, plus a richer boot sequence (module-loading preamble with pixel progress bars, the typed `$ whoami` exchange, an `operator --info` line carrying the identity hook). Fills most of the viewport — Home is Hero-only, see structural-redesign revision note. **The literal boot-sequence device (typed lines, chrome-buttons bar) is Hero-exclusive** — the surrounding pixel-art system (rivets, cursor, scanlines) is shared site-wide as of the big visual pass.
...
- **Capabilities** — "what I do," on `/approach` (moved off Home in the structural redesign). Grounded in `BRANDFINAL.md` §1 "What I do" — links to `/track-record`. Rendered as a boot-menu: three `<details>`/`<summary>` items (Cleansing & migration / Warehousing & BI / Agentic AI systems), pixel-arrow selector, description revealed only for the open item.
- **CaseStudyCard** — fields: client/sector (or anonymized descriptor), problem, approach, outcome, tags[]. Single variant (always full detail) — used only on `/track-record`. Populated with three real, anonymized entries as of the real-content pass (life sciences, public sector, fitness); more get added by hand, one at a time, only from real details the user supplies. No Retail/Education entries — no real detail given yet.
